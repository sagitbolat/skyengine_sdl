...
namespace Noise {
    ...
    static float amplitude = 1.0f;      // Amplitude of the first octave.
    static float lacunarity = 2.0f;     // Scaling factor of the frequency for successive octaves.
    static float persistence = 0.5f;    // Scaling factor of the amplitude for successive octaves.
    
    inline int32_t fast_floor(float f) {
        int32_t i = (int32_t)f;
        return (f < i) ? (i - 1) : (i);
    }
	
    static const uint8_t perm[256] = { 151, 160, 137, 91, 90, 15,
